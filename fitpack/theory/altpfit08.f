...
C
     ...
     2,LABEL,LABEL2
      ...
      integer len,jtgt,ioff

      logical set
      dimension datchis(30),idatpts(30)
      double precision normchis(30),normtot
...
*     vars for pdf error bands
      character*40, file1
      character*4, tmp
      dimension derv(-5:2,30,35), err(-5:2,30), pdf0(-5:2,30)
     &     , xgrid(30),q2grid(30)
...
C
 ...
 61      format(1h ,'  Off-shell corr. by Kulagin-Petti NOT YET CODED')
...
*    ... data chi^s
 ...
 317  format(/,' data set          chisq      npts  chisq/pts'
     ...
     &        ,datchis(i)/idatpts(i)
      ...
      write(6,319)
 318  format(A10,F15.2,I8,F9.2)
 319  format('---------------------------------------------')
      WRITE(6,318) 'TOTAL     ',CHITOT,NPTS,chitot/npts
      WRITE(6,318) 'TOTAL+norm',CHITOT+normtot,NPTS
     &     ,(chitot+normtot)/npts
...
*     ... write to file
         DO  j = 1, nq2
            WRITE(3,5031) q2grid(j),(pdf0(ipt,j),err(ipt,j),ipt=2,-5,-1)
         end do
