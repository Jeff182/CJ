...
*     vars for pdf errors
      character*80, file1
      character*4, tmp
      dimension derv(-5:4,50,50), err(-5:4,50), pdf0(-5:4,50)
     &     , xgrid(50),q2grid(30)
...
C
      ...
      read(10,*) xmb,ncb
c
c  nofit = 0  set all parameter errors = 0 forcing a single pass output
...
*             ... writes DIS data 
 ...
 88               format (2F7.2,3x)
               ...
               if (it.lt.80) then ! structure function output
                  write(IUout,84) xb,Q2,W2,th,sig
     2                 ,xerr(k),chiout,ititle(data(6,k))
 84               FORMAT(e9.4,F10.3,E11.3,2E14.5,x,E14.5,a17,a10)
               else
                  write(IUout,85) xb,Q2,y,W2,th,sig
     2                 ,xerr(k),chiout,ititle(data(6,k))
 85               FORMAT(e9.4,F10.3,F7.3,E11.3,2E14.5,x,E14.5,a17,a10)
               end if
            else if (k.le.NDIS+NDY) then
*             ... writes DY data
...
c    *** Prints KP off-shell parameters if ioff=3
      if (ioff.eq.3) then
         write(iuout,*)
         write(iuout,*)
...
*    *** writes PDF parameters, sample PDF and errors to ##.pdf ##.par files 
      if (nofit.gt.0) then      ! but only if a fit was performed
...
*       ... PDF at given Q2 vs. x
 ...
 5103       FORMAT(//,' Q2=',F10.2)
            WRITE(3,5104)
 5104       FORMAT(/,3X,' x',9X,'db/ub',16X,'d/u',17X,'xd',18X,'xu'
     2           ,18X,'xg',18X,'xub',17X,'xdb',17X,'xs',18X,'xc'
     3           ,18X,'xb')
...
*          ... write to file
     ...
     &                 ,(pdf0(ipt,j),err(ipt,j),ipt=4,-5,-1)
 5031             FORMAT(E10.3,26E10.3) 
 5021          CONTINUE
...
*          ... PDF at given x vs. Q2
 ...
 5203          FORMAT(//,'  x=',E15.3)
               WRITE(3,5204)
 5204          FORMAT(/,3X,'Q2',9X,'db/ub',16X,'d/u',17X,'xd',18X,'xu'
     2              ,18X,'xg',18X,'xub',17X,'xdb',17X,'xs',18X,'xc'
     3              ,18X,'xb')
               do j=1,nq2
                  Q2 = q2grid(j)
...
c$$$            endif
            ...
            if((w2.lt.w2lo(j)).or.(w2.lt.w2lo(j)))goto 200
            npts=npts+1
...
c$$$            endif
         ...
         else if(itype(j).ge.80.and.itype(j).le.95)then
c
c  HERA Combined data for NC or CC
C  extended to cover all cross-section data, range 80-95
c
