...
c
c  Approximate target mass corrections added 5/5/08
c
...
C  ALTERATIONS ARE MADE.
         ...
         ELSE IF(IT.GE.126.and.IT.LE.130)THEN
            S=V(1)
            Y=V(3)
            RS=SQRT(S)
...
C     CG2=2.*FAC*(6.*Y*(1.-Y)+(1.-2.*Y+2.*Y*Y)*DLOG(1./Y-1.))
      THEORY=THEORY+.5*(1.-X)*WI(I)*AL*(C22*FTEMP(1)+C23*(FTEMP(1)-FX))
      if(istruc.lt.3)THEORY=THEORY+.5*(1.-X)*WI(I)*AL*CG2*FTEMP(2)
   23 CONTINUE
      if(istruc.eq.1)theory=theory/2./x
      RETURN
      END 
