...
*            print*, '*1 ans =',ans,ndrv,vp
         else if(nfl.eq.2)then
*          ... F2(e+D)
	    call deuteronSF(1,2,itg,itm,ihtfl,ndrv,vp,xc,ans)
         else if(nfl.eq.5)then
*          ... F2(nu+D) == [F2(nu)+F2(nubar)]/2
            az=0.5
            an=0.5
	    call deuteronSF(2,2,itg,itm,ihtfl,ndrv,vp,xc,ans1)
	    call deuteronSF(3,2,itg,itm,ihtfl,ndrv,vp,xc,ans2)
c$$$            if(itg.eq.3)then
c$$$*             ... Kulagin-Petti nuclear corrections 
c$$$*             ... [AA Feb 09] WHY use Fe corrections on deuteoron targets??
c$$$               ans1=ans1*nuke_cteq(v(1),v(2),2,0,2,3,2)
c$$$               ans2=ans2*nuke_cteq(v(1),v(2),2,0,-2,3,2)
c$$$            endif
            ans=(ans1+ans2)/2.
         else if(nfl.eq.8)then
*          ... F3(nu+D) - no nuke correction
            az=.5
            an=.5
...
c$$$            if(itg.eq.3)then
c$$$*             ... Kulagin-Petti nuclear corrections 
c$$$               ans1=ans1*nuke_cteq(v(1),v(2),3,0,2,3,2)
c$$$               ans2=ans2*nuke_cteq(v(1),v(2),3,0,-2,3,2)
c$$$            endif
            ans=(ans1+ans2)/2.
