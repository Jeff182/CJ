      ...
      real*8 nuke_cteq
      DIMENSION V(4),XC(60),GF(8,40,1800)
...
C          ... first compute F_2(D)
	    call deuteronSF(1,2,itg,itm,ihtfl,ndrv,vp,xc,F2deut)
C          ... CALCULATE F2 PROTON HERE
            az=1.
            an=0.
            CALL strfn(1,2,itm,ihtfl,NDRV,VP,XC,F2P)
...
*          ... F2(e+n)
            az=0.
            an=1.
            call strfn(1,2,itm,ihtfl,ndrv,vp,xc,ans)
         else if(nfl.eq.1)then
*          ... F2(e+p)
            az=1.
            an=0.
            call strfn(1,2,itm,ihtfl,ndrv,vp,xc,ans)
...
*          ... F2(e+D)
	    call deuteronSF(1,2,itg,itm,ihtfl,ndrv,vp,xc,ans)
         else if(nfl.eq.5)then
*          ... F2(nu) == [F2(nu)+F2(nubar)]/2
*          ... Only for D or Fe targets
            if (itg.ge.0) then 
               iitg = itg
            else
               iitg = 0
            end if
	    call deuteronSF(2,2,iitg,itm,ihtfl,ndrv,vp,xc,ans1)
	    call deuteronSF(3,2,iitg,itm,ihtfl,ndrv,vp,xc,ans2)
            if(itg.eq.-3) then
...
c     modified to include TMC or HT corrections.
            ...
            if(itg.eq.-3)then
               az=0.5
               an=0.5
               call strfn(2,1,itm,ihtfl,ndrv,v,xc,ans1)
               call strfn(2,2,itm,ihtfl,ndrv,v,xc,ans2)
               call strfn(2,3,itm,ihtfl,ndrv,v,xc,ans3)
               ans1=ans1*nuke_cteq(x,q2,1,0,2,3,2)
               ans2=ans2*nuke_cteq(x,q2,2,0,2,3,2)
               ans3=ans3*nuke_cteq(x,q2,3,0,2,3,2)
            else if (itg.eq.-1.or.itg.eq.-2) then
               az=0.465
               an=0.535
               call strfn(2,1,itm,ihtfl,ndrv,v,xc,ans1)
               call strfn(2,2,itm,ihtfl,ndrv,v,xc,ans2)
     ...
     2          +y*(1-y/2.)*ans3
            ...
            if(itg.eq.-3)then
               az=0.5
               an=0.5
               call strfn(3,1,itm,ihtfl,ndrv,v,xc,ans1)
               call strfn(3,2,itm,ihtfl,ndrv,v,xc,ans2)
               call strfn(3,3,itm,ihtfl,ndrv,v,xc,ans3nubar)
               call strfn(2,3,itm,ihtfl,ndrv,v,xc,ans3nu)
               ans1=ans1*nuke_cteq(x,q2,1,0,-2,3,2)
               ans2=ans2*nuke_cteq(x,q2,2,0,-2,3,2)
     ...
     2                         -ans3nu*nuke_cteq(x,q2,3,0,2,3,2)
               ...
               call strfn(3,1,itm,ihtfl,ndrv,v,xc,ans1)
               call strfn(3,2,itm,ihtfl,ndrv,v,xc,ans2)
               call strfn(3,3,itm,ihtfl,ndrv,v,xc,ans3)
            endif
            ans=(1.-y-(.938*x*y)**2/q2)*ans2+y**2*x*ans1
     2          -y*(1-y/2.)*ans3
            ans=ans*1.5816/(1.+q2/80.22**2)**2
...
C  ALTERATIONS ARE MADE.
         ...
         ELSE IF(IT.GE.126.and.IT.LE.131)THEN
            S=V(1)
            Y=V(3)
            RS=SQRT(S)
...
*       istruc    = (i)  structure fn: 1=F1 2=F2 3=xF3  
*       itm       = (i)  TMC  0=no  1=yes
*       iht     = (i)  HT   0=no  1=yes
*       ndrv      = (i)  ??????????
*       v(4)      = (dp) variables - for DIS v(1)=xB  v(2)=Q^2  v(3,4) unused
*       xc(60)    = (dp) current fit parameters
*
*     OUTPUT:
*
*       sfn        = (dp) The required structure function
*
*     EXAMPLE calling sequence for elm proton F2
*
*     az = 1.          ! fraction of protons 
*     an = 0.          ! fraction of neutrons
*     call strfn(1,2,NDRV,V,XC,sfn)
*
*     TO DO LIST (28 Nov 08)
*
*       - HT for F1, F3  (for the moment they are == HT for F2)
*       - FL 
*       - cross sections
*
      implicit none
      
      integer ibeam,itarg,istruc,ndrv,itm,iht
      double precision v(4),xc(60),sfn

      double precision vp(4),x,q2,xtmc,amu,r,fLcor,f1cor,f2cor,f3cor,xht
     &     ,ans,ht_f1,ht_f2,ht_f3,ht_fL

*     Higher-twist parameter indexes
      integer nht(4)
      common/ht/nht

*      integer itype(3),nflag(30),inorm(30),itgt(30),icorr(30),itmc(30)
*     &     ,iht(30)
*      common/flags/itype,nflag,inorm,itgt,icorr,itmc,iht
...
*                       with same ht_f2 as iht-10
      ...
      if (iht.eq.0) then
         ht_f2 = 0d0
      else if (iht.eq.1) then ! a * xB^b * (1+c*xB+d*xB^2) / Q^2
         ht_f2 = xc(nht(1)) 
     &        * xht**xc(nht(2)) 
     &        * (1d0 + xc(nht(3))*xht + xc(nht(4))*xht*xht)
*     &        * (1d0 + xc(nht(3))*xht) * (1-xht)**xc(nht(4))
     &        / Q2
      end if
*    ... HT for FL, F1 and F3 == HT for F2, for the moment (Feb 09)
      ht_fL = ht_f2
      ht_f1 = ht_f2
      ht_f3 = ht_f2
...
*    ... NOTE: theory02 always correct for proton/neutron ratio
*    ... (the common vars az=Z/A ans an=1-Z/A must always be set 
*    ... prior to any call, also for a proton)

      call theory2(ibeam,2,istruc,ndrv,vp,xc,ans)

      if (istruc.eq.0) then
         sfn = ans * fLcor * (1d0 + ht_fL)
      else if (istruc.eq.1) then
         sfn = ans * f1cor * (1d0 + ht_f1)
      else if (istruc.eq.2) then
         sfn = ans * f2cor * (1d0 + ht_f2)
      else if (istruc.eq.3) then
         sfn = ans * f3cor * (1d0 + ht_f3)
      end if

*      print*, '*2 ans,sfn,f2cor,f3cor =',ans,sfn,f2cor,f3cor
...
*    *** tree level str.fn. 
      ...
      IF(IORD.EQ.0)then
         ...
         if(istruc.eq.0)theory=0d0
         return
      endif

*      print*, '*3 ibeam,itarg,istruc,ftemp LO ='
*     &     , ibeam,itarg,istruc,theory
*      print*, '*4 x,s,Q02,Q2 =',x,s,Q02,Q2
*      print*, '*5 ndrv =',ndrv

*    *** 1 loop contribution
c
c  flags for F1, xF3 or FL
c
      ff1=0.
      ff2=1.
      ff3=0.
      if(istruc.eq.0) then
*       ... massless FL = -F1 + F2/(2x) according to def by 
*       ... Aivazis,Olness,Tung,PRD50(1994)3085
         ff1 = -1.
         ff2 = -1. + 0.5/(x*x)
      else if(istruc.eq.1)then
         ff1=1.
...
c
      ...
      DO 23 I=1,NTERMS
         ...
         C22=CF*(ff2*(6.+4.*Y-2.*(1.+Y*Y)/(1.-Y)*DLOG(Y)-2.*(1.+Y)*AL1)
     2        -ff3*2.*(1.+Y)-ff1*4.*y)
         C23=CF*ff2*(-3.+4.*AL1)/(1.-Y)
         CG2=2.*FAC*(ff2*(-1.+8.*Y*(1.-Y)+(1.-2.*Y+2.*Y*Y)
...
*      print*, '*6 LO, ftemp(1), theory =',FX,ftemp(1),theory
...
C  THIRD DIMENSION OF GF (1800 ABOVE). THIS IS 50+50*(NO. OF VARIED PARTON 
C  PARAMETERS).
C
      IS=S/DELTA+1
      isign=1
...
c  w mass
      ...
      if(it.eq.131)then
         ...
         xa0=r*exp(y)
         xb0=r*exp(-y)
         CALL HOQUARK(1,16,XA0,ALS,NDRV,QA)
         CALL HOQUARK(1,16,XB0,ALS,NDRV,QB)
...
C      call dist(xb,qb)
  ...
  100 continue
      ...
      if(it.eq.126)then
         ...
         ansp=ansp*CDF1800(y)
         ansm=ansm*CDF1800(-y)
      else if(it.eq.128.or.it.eq.130)then
         ansp=ansp*CDF1960(y)
         ansm=ansm*CDF1960(-y)
