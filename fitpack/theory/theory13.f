     ...
     3,itmc(49),iht(49), tv(4),tmp(1008), sumd0(36)
      dimension xsect1(1188,3),xsect2(3240,3),xsect3(3960,3),xsect4(2592,3)
     ...
     *IFREP(50),NPARAM,NVAR,IREJ
      ...
      common/C_array/cv(0:35), csq(0:35), cg(0:35), xlambda(0:35)
...
c
    ...
    3 CONTINUE
      NDRV=NDRV+1
      IF(IFREP(NDRV)-35)7,7,5  ! If 'PDF' parameters (NDRV<=35) is changed
                               ! performs QCD evolution, else proceeds
    7 CONTINUE
      S0=DLOG(Q02/XC(1)**2)
...
C
      ...
      call init_offshell ! initializes the offshell mods for PDFs 
                         ! (for fmKP model they are PDF dependent:
                         ! need to be recalculated every time the PDFs 
                         ! for other models the call has no effect.)
      cv(ndrv)=c_v
      csq(ndrv)=c_sq
      cg(ndrv)=c_g
      xlambda(ndrv)=lambda
      print*,ndrv,cv(ndrv),csq(ndrv),cg(ndrv)
c
c Normalizations and offshell strength are now stored for each value of ndrv
c jfo 3/25/15 They may be accessed through the common block C_array
c
...
c
      ...
      else if(it.eq.80.or.it.eq.81.or.it.eq.84)then
c     it=80 is electron neutral current
c     it=81 is positron neutral current
         az = 1.                ! proton target
         an = 0.
         x=vp(1)
         Q2=vp(2)
...
c            print*,x,Q2,ans,ansf2,ans0
      ...
      else if(it.eq.82.or.it.eq.83)then
         az = 1.                ! proton target
         an = 0.
         x=vp(1)
         Q2=vp(2)
         ...
         ym=1.-(1.-y)**2
c
c  it=82 is electron charged current which is like antineutrino 
c  scattering -->ibeam=3 
c  it=83 is positron charged current which is like neutrino 
c  scattering -->ibeam=2
c
         ibeam=3
         if(it.eq.83)ibeam=2 
...
c
      ...
      if (ism.le.1) then           ! no smearing, no offshell
         ...
         DO I=1,NTERMS
            ...
            if (ioff.eq.5) then                          ! fmKP off-shell model
     ...
     &                     sbop,cbop,bbop,glueop)
            ...
            if (xoy.lt.0.995d0) then
               ...
               if (ioff.eq.5) then !fmKP offshell correction
         ...
         ENDDO
         if (ioff.eq.5) then         ! apply offshell corrections to pdfs
            u=u+uo
            d=d+do
            ub=ub+ubo
            db=db+dbo
