...
*           calling routine.
*
*     NOTE: theory2 is called with ots argument itarg=2, so that proper 
...
*
     ...
     &     ,ans,ht_f1,ht_f2,ht_f3,ht_fL,C(0:7),HT(0:7)
...
*    ... multiplicative parametrization
     ...
     &        / Q2
         C(5) = xc(nhtn(1)) 
     &        * xht**xc(nhtn(2)) 
     &        * (1d0 + xc(nhtn(3))*xht + xc(nhtn(4))*xht*xht)
     &        / Q2
...
c         FAC=FLAVOR
         FAC=4.
         IF(ibeam.eq.1)fac=10./9.+(flavor-4.)/9.  ! for gamma str.fns
         if(ibeam.eq.4)fac=1.
         FX=THEORY
...
c
      if(ibeam.eq.1)then
         ...
         if(istruc.eq.2.or.istruc.eq.0)then
     ...
     &           *(vd**2+ad**2)*pz**2
      ...
      if(ibeam.eq.4)then
         ftemp(2)=ftemp(2)*(2.*a0u+2.*a0d+(flavor-4.)*a0d) * (aaz + aan)
      endif
      return
      end
