...
c
c jfo - removed strangeness supression factor as xp(34) is used elsewhere
c
      return
...
*     dbar/ubar -  parametrization at Q0
      ...
      common/param/xp(100)
      common/iparam/iparamc
      if(iparamc.eq.1.or.iparamc.eq.6)then
...
c
      ...
      else if (iparamc.eq.7)then
         fdboub=xp(20)*x**(xp(21))*(1.-x)**xp(22)
c     $   +(1.+xp(23)*x)*(1.-x)**xp(24)
     $   +1.+xp(23)*x*(1.-x)**xp(24)

      else if(iparamc.gt.1.and.iparamc.le.5)then
c         fdboub=1.+xp(20)*x**xp(21)*(1.-x)**xp(22)
c     $            +xp(23)*x*(1.-x)**xp(24)
         fdboub=(fubpdb(x)+fdbmub(x))/(fubpdb(x)-fdbmub(x))
c      else if(iparamc.eq.5)then
