...
deuteron (``smearing function'') \cite{MSToff, KPW94}.  
...
While the off-shell nucleon PDF $\widetilde{q}^N$ is not by itself
an observable, its dependence on the virtuality $p^2$ can be studied
within a given framework.  Since the bound state effects in the
deuteron are the smallest for all atomic nuclei, one can expand the
off-shell nucleon distribution about its on-shell limit \cite{KP06},
%
\begin{eqnarray}
...
\end{eqnarray}     
%
where the coefficient of the off-shell term is formally given by
%
\begin{eqnarray}
\delta f^N(x,Q^2)
&=& \left.
...
Defining the total quark PDF in the deuteron by
$q^d = q^{d\, (\rm on)} + q^{d\, (\rm off)}$, the on-shell and
off-shell contributions can be written
%
\begin{subequations}
\begin{eqnarray}
q^{d\, (\rm on)}(x,Q^2)
...
\end{subequations}
%
In practice we compute these within the weak binding approximation
(WBA) in terms of the deuteron wave function \cite{KP06, KMK09}.
