...
\end{eqnarray}
%
one can infer that the function $\delta f^N$ must have one
or more zeros in the physical range between $x=0$ and 1.
%
...
To test the sensitivity of the fit to the off-shell model,
we also consider the more microscopic OCS model for the off-shell
function $\delta f^N$ discussed in Sec.~\ref{sssec:offshell}.
...
For the AV18, CD-Bonn and WJC-2 wave functions, the effects
of the more restrictive OCS model and relatively small and
generally within the $\Delta\chi^2=1$ bands.
...
\caption{Lepton charge asymmetry $A_{\ell}$ from
	...
	D\O\ electron (blue circles) \cite{D0_e} and
	muon (cyan triangles) \cite{D0_mu} data
