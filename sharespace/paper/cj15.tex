...
correction $q^{d\, (\rm off)}$ in Eq.~(\ref{eq:qdoff}) is much more
model dependent.  In the literature a number of model studies have
been performed to estimate the modification of PDFs in bound nucleons
relative to the free nucleon PDFs \cite{GL92, MSToff, KPW94, MSTplb,
KMPW95, MSS97, KP06, Mineo04, Cloet06}, some of which have been
motivated by the original observation of the nuclear EMC effect.
%
...
covariant spectator quark models \cite{MSToff, KPW94, MSTplb, KMPW95,
KP06}, which typically assumed a fixed mass of the spectator ``diquark''
system, and a simple quark--nucleon correlation function regularized
by cutoff parameters fitted to reproduce the $x$ dependence of
on-shell PDFs.
%
The possible effects of nucleon off-shell corrections on global PDF
analysis were first explored in Ref.~\cite{CJ10} using a simple
parametrization of the corrections computed in the quark spectator
model of Ref.~\cite{MSTplb} for $x \gtrsim 0.2$.
%
...
The change in the confinement radius (or nucleon ``swelling'')
was varied between 1.5\% and 1.8\%, and the virtuality of the bound
nucleons
...
Most recently, the CJ12 global analysis \cite{CJ12} further took into
...
This full range was consistent with the previous experimental data,
with each set giving essentially the same $\chi^2$ values for the
global fit, $\chi^2/{\rm dof} \approx 1.03$.
...
Note that in all the previous studies the off-shell corrections were
applied only to the deuteron $F_2^d$ structure function and in the
valence quark approximation.  In a recent extension of this model,
Ehlers {\it et al.} \cite{Ehlers14} generalized the off-shell
modifications to apply also to antiquarks and gluons, with an
appropriate choice of spectator quark masses, allowing the
corrections to be applied to any observable.
